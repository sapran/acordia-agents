> **RETIRED in 5.0.0.** The operations pillar was removed and the distribution now ships the
> Analysis pillar alone. This file is kept, moved under `docs/roles/archive/`, as the sole record
> of what was ported from the CyberStrike fork at commit `359655518` and where it deliberately
> diverged. Nothing in the shipped tree derives from it any longer. See `openspec/changes/` for the
> `analysis-only` change that retired it.
