...
The picture is a target model, not a pile of facts: it must be shared with the people who collect against it and with the human operator who must act. A gap is an explicit unknown in that model, not a general demand for more data (`Clark`, pp. 20, 169). The grid's tasking method expresses each gap as a question whose answer could change a decision. Tooling may surface correlations, scores, or a proposed explanation, but it has made a hypothesis, not a conclusion: the analyst owns the judgement only after retaining the evidence, stating confidence and provenance, and testing alternatives (`Cvetko-Davydiuk`, pp. 13, 15). Access and collection can make an operation more capable without themselves establishing strategic success; that connection remains a case-by-case judgement (`Lindsay`, intelligence performance).
...
**A lead that cannot dispatch is not a lead, and it establishes that positively rather than noticing it.** The seat's first act is to enumerate its own tools and state whether the dispatch tool is among them, because determining what one lacks is part of the analytic work and not a preliminary to it — the operational analyst "must not only analyze the information they have, but also determine what they are lacking" (Monte, *Network Attacks & Exploitation: A Framework*, document `c159a333`, p. 60). Phrasing the guard as an absence does not survive contact: a seat told to stop *if* it cannot dispatch enumerated its tools, held no dispatch tool, and never once reached for the guard, because an absence nobody asked it to look for is one it does not notice. What that silence costs is the pillar itself — ACORDIA makes Analysis "an explicit core activity rather than an implicit component of 'People' or 'Tools'" (Styran & Yashchuk, document `1152d85c`, p. 20), and a lead quietly doing four specialists' work has hollowed out that activity while the product still reads as though four seats filled it.
Before directed credential collection, refresh the operation's carried knowledge, map the technical terrain and systems, and connect those assets to mission threads and crown jewels before deciding credential priorities. The lead dispatches every independent, bounded orientation question together, including disjoint slices owned by one leg, rather than treating the roster as a fixed pair. Every assignment has a unique brief slug, carried into its notes and credential-file names. It waits only where the next question consumes a return: Mission values the asset register after it exists, and credential prioritisation waits for the orientation returns it needs. The lead holds the fused asset-and-credential hypothesis register; collection is then targeted to the systems it names, followed by a residual pass for systems the orientation did not explain.
...
**The return path is bounded; the work itself is not.** That same weak point governs the leg's reply, and it fails in its own way: what is handed back is cut to whatever the channel between the two seats carries, without warning, so neither the analyst who wrote the read nor the lead who receives it learns that the end is missing. The failure has two halves and so does the remedy. What is not explicitly recorded in reporting is exactly what a later reader needs to make sense of it (Lindsay, *Information Technology and Military Power*, p. 172) — so the full working, with its evidence, identifiers, queries, dead ends and what was deliberately not done, is written where the work was done, in the task's own directory, as a record made at the time rather than reconstructed afterwards from memory (Pherson & Heuer, p. 547). But an administrative load aggregated and sent forward at every echelon is itself friction (Lindsay, p. 172) — so what travels up is only the judgement, its confidence, its gaps, and the name of the file holding the rest. How much may travel is stated by the dispatching brief and settled before the work begins, in the way procedure words are agreed by planners rather than invented while transmitting (Lindsay, p. 178). Where a read does not fit, the question was too large, and saying so is itself the finding.

**The reader must not have to reconstruct the analyst's work.** A finished report is repository-directed production practice: expand an acronym or initialism on its first reader-visible use, collect every used term in a separate report-language acronym chapter, use a labelled table where related findings share attributes, and place a usable evidence reference beside every evidence-backed claim. When a term's full form or meaning is unestablished or ambiguous, the report says so rather than guessing. The rules make comparison and inspection possible without turning every finding into a table or every claim into a quotation. A matrix sorts and organises data for comparison and analysis (`SAT` §5.3, p. 149); transparent analysis names its assumptions, inference chain and uncertainty (`Heuer`, p. 44); and a practitioner account directs readers to its glossary and acronym list (`grugq-Ukraine`, pp. 11, 26). Those works ground the rationale, not these exact mechanics: the first-use, chapter, table and claim-reference rules are this repository's user-directed reporting practice.

**A source a reader can open is part of the report, not a later convenience.** Where the issuing evidence system has a canonical safe locator already known to the analyst, the claim-local reference is navigable; where it does not, the report names the unavailable locator rather than inventing one. The particular route and rendering check are procedural detail owned by the skill for that evidence system.
...
- Hutchins, Cloppert & Amin, *Intelligence-Driven Computer Network Defense… Kill Chains*, pp. 2, 5; Lindsay, J. R., *Information Technology and Military Power*, pp. 166, 172, 178, 179 (pp. 172, 178: unrecorded context and reporting load; procedure words agreed in advance — the hand-back contract).
- Heuer, R., *Psychology of Intelligence Analysis*, pp. 150–151; Pherson & Heuer, *Structured Analytic Techniques for Intelligence Analysis*, pp. 47, 69, 77, 87, 117, 547 (p. 117: explicit communication protocols where face-to-face contact is absent; p. 547: the written record made at the time rather than recalled — the hand-back contract).
