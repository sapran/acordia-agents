...
- **`acordia-operators/`** — the ACORDIA Operations pillar. One primary orchestrator (`operator`) plus four subagent specialists (`web-application`, `mobile-application`, `cloud-security`, `internal-network`), a 30-skill technique library ported from the CyberStrike fork (commit `359655518`), and 9 command wrappers.
...
- Operator prompts record state under `## Operation journal`, naming the `.acordia/ops/` files (`scope.md`, `intel.md`, `coverage.md`, `findings/<slug>.md`, `reports/<name>.md`) in prose.
