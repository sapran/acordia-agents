...
- Every prompt carries a `## Guardrails` section stating the current posture: **write freely** — notes, working files, drafts, product — and **do not modify the material given for analysis**; evidence, collected data, logs, dumps and captures are read-only inputs, and derived work goes in the agent's own files, never back over the source. `.acordia/` is named as the root those files belong under — `.acordia/work/` for the task directory, `.acordia/reports/` for a finished product, which carries the task directory's `<corpus>-<YYYY-MM-DD>-<task-slug>` stem so two corpora worked on one day cannot collide — **by convention, not by permission**. A prompt names the sink where it states the instruction rather than repeating both everywhere: the four legs name the root in `## Guardrails`, while the orchestrator names `.acordia/work/` in `## A directory per task, a bound per reply` and only `.acordia/reports/` in its own — see `docs/implementation-notes.md`. `briefing-reporting` is the one place the naming scheme is stated; a prompt names the parent and defers the rest to it. It closes with execution belonging to the operators the analyst advises — a human, since the distribution dispatches no one — and no prompt may claim to hold no file-editing tool.
