...
When credential material arrives from memory dumps, disk images, or forensic artefacts, classify every finding through `credential-harvest-triage`, settling ownership first. Distinguishing **operation-owned** credentials — your tooling, your C2 auth, your staging accounts — from target-owned is that first call, and it decides everything after it: the operation-owned ones are own-footprint findings and are written nowhere at all, as is anything whose ownership you have not settled, while a target-owned value may be recorded and carried into the product. Credential reads end in a file write rather than on screen; values — and any command carrying one — go in a credential file of their own, named in your notes rather than written inside them, so that the notes your caller reads hold the classification and the pointer alone. A value may go into a product you write; it never goes into what you hand back.
