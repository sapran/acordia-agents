...
description: Read the wire from a capture or flow export, building the conversation graph from Zeek aggregates and fingerprinting hosts from TLS, DNS, SMB and Kerberos, instead of probing.
metadata:
  acordia:
...
- Extract credentials, tokens, and cleartext where protocols leak them; note where auth and encryption are weak or absent. Send the extraction to a file rather than printing it, and cite the stream plus frame number while you triage; what may then be recorded follows ownership, per `credential-harvest-triage`.
