...
description: Model a target's AWS, Azure or GCP estate at the control plane from collected exports — IAM policies, service principals, federation — when administration lives in the API.
metadata:
  acordia:
...
- Cloud-plane credentials frequently have session/time bounds — record `Expiration` in `freshness`; expired credentials still classify for correlation but priority drops to P3. Root/organisation-level keys (`arn:aws:iam::*:root`, GCP `roles/owner`, Azure `Global Administrator`) always mark P0. Reporting via [`credential-harvest-triage`](../credential-harvest-triage/SKILL.md), ownership settled first — a service-account key collected from your own staging tenant is operation-owned and is written nowhere. Send state-file and secret-store extractions to a file rather than to standard output, and let the IAM path or ARN identify the credential while you triage it.
