...
metadata:
  acordia:
    ...
    grid_row: null
    procedural: true
    source: openspec/changes/archive/2026-07-22-exhaustive-data-processing/proposal.md
---
...
- **Credential values follow ownership** (see `credential-harvest-triage`): a target-owned one belongs in a credential file of its own and in the product, while your own and anything not yet settled belong nowhere. Whichever it is, send a scan's hits to a file in your task directory rather than letting them print — a sweep across a corpus matches in bulk, and a printed one puts every hit into the session record at once.
