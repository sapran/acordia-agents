...
**Adding vocabulary widens; only structure narrows.** The 66% rule in step 4 is why: every extra synonym is another term that need not match. That rule's "precision comes from `filter:`, always" holds for *words*; a structural marker is the other way to narrow `q` itself, because ANDed rare tokens are not more vocabulary. The same intent as the tight form above, spread across vocabulary — `(PrivateKey OR PresharedKey OR "[Interface]") AND (впн OR vpn OR amnezia OR wireguard OR туннел)` — returned 4,406, dominated by web pages: 3,592 of them, against 219 tables. This is language-independent: translating the synonym list does not rescue it, and a corpus in a language you are working through raises the temptation rather than the yield.
