...
1. **Scope to a collection before anything else.** `collection` is a required argument on `search_entities` and `match_entity`, and it is the same word wherever scope is named — `get_collection`, `list_entitysets`, `xref_results`. It takes a numeric id (`"874"`), a `foreign_id` (`"68c4558f5506410ba3e32fce80fe690c"`) or a list of either; foreign ids are resolved server-side and cached, so no conversion call has to come first. The literal `collection="*"` is the only way to search every readable collection, and a reply at that scope carries a `_note` beginning `EVERY COLLECTION:`, so an instance-wide search is something you chose rather than something you drifted into. **Scope is not a filter** — `collection_id` inside `filters` is refused with an error naming the argument. Then read `searched.collection` on the reply: it reports the scope actually applied, as resolved numeric ids or `"*"`, and a `foreign_id` or a list is worth confirming rather than assuming.
