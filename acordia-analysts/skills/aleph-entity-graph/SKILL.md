...
- **The identifier argument is `entity_id`, on every tool that takes one** — `get_entity`, `get_entity_text`, `expand_entity`, `entity_tags`, `similar_entities`. Not `id`, and not `entity`. A profile is `profile_id`. An unrecognised key is dropped rather than refused, so the call either fails naming a missing `entity_id`, or reaches the server and returns a not-found that reads like a bad identifier instead of a bad call — five reads were lost this way across two runs before the pattern was noticed.
- **Build the argument object in code, not by hand.** A `q` carrying quoted phrases joined by `OR` needs every inner quote escaped, and a hand-written object reliably escapes the first phrase and then stops — six searches were lost to exactly that in one run, each on its second, non-English phrase. Construct the object with a JSON serialiser inside a script and the escaping stops being yours to get wrong; per `analytic-tooling-scripting` that is also the path where one loop replaces twenty interactive calls.
