...
- **Give the task its own directory.** Where a dispatch names no working directory, create one under a short dated slug and say where it is. It holds a `README.md` carrying the request **verbatim**, its date, and one line on what is being settled — a record made at the time rather than reconstructed afterwards, so that a later reader has what the work was answering and not only what it concluded.
