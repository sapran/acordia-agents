...
description: Reverse-engineer an implant or payload binary to ground-truth what it does on a host and over the wire, when validating your own tooling or reading a captured sample.
metadata:
  acordia:
...
- Extract config and IOCs — keys, domains, mutexes, campaign markers — and map behavior to ATT&CK techniques and the telemetry each would produce. Cite each finding as `<sample>:<offset>` for binary evidence or `<trace>@L<line>` for sandbox log lines; send decrypted config to a file rather than printing it, and settle ownership before any of it is kept — a key embedded in a sample of your own tooling is operation-owned and is written nowhere, per `credential-harvest-triage`.
...
- `strings -a -n 8` (and `strings -e l` for UTF-16 on Windows PE), then pattern-library grep from [`credential-harvest-triage/references/credential-patterns.md`](skill://credential-harvest-triage/references/credential-patterns.md), redirected to a file rather than left to print. Hits often surface C2 auth keys, hardcoded RDP/SMB credentials, hardcoded API tokens for stagers pulling from paste sites — and in a sample of your own tooling those keys are yours, so settle ownership before any of it is kept.
...
- Payload-embedded credentials frequently have *very* broad `reuse-potential` inside their intended deployment scope (all bots authenticating to the same C2, all sample instances sharing an XOR key) but classifying reuse against the target requires input from `terrain-analyst`. Classification and reporting via [`credential-harvest-triage`](../credential-harvest-triage/SKILL.md), ownership settled first — a key embedded in a sample of your own tooling is operation-owned. Send `strings` and decompiler output to a file rather than to standard output, and let the offset within the binary plus a decryption-method identifier carry the finding while you triage it.
