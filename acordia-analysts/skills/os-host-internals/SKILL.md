...
- OS-store credentials classify by user scope (`scope: account` or `host`), except LSA/machine-account material which is `scope: host`+ (can pivot to domain via silver-ticket-style analysis — flag for `identity-directory-trust`). Reporting via [`credential-harvest-triage`](skill://credential-harvest-triage), ownership settled first. Send store extractions — `secretsdump`, `chainbreaker`, a keyring parse — to a file rather than to standard output, and let the path within the collected image identify the finding while you triage it, redacting your own home directory.
