...
description: Read a host's disk and memory as a responder would, driving Volatility walks and parsing filesystem timelines, registry and event logs, when checking what you left.
metadata:
  acordia:
...
- Every extraction is classified and reported via [`credential-harvest-triage`](skill://credential-harvest-triage), ownership settled first. Send the parser's output to a file rather than to standard output — a printed dump lands in the session record as well as on your screen — and work from the receipt.
