...
- Directory-derived credentials classify as `scope: domain` or higher; `krbtgt`, tier-0 accounts, and enterprise-CA private keys always mark `priority: P0`. All output flows through [`credential-harvest-triage`](skill://credential-harvest-triage), ownership settled first; `secretsdump` and its kin write to a file rather than to standard output, and the account name plus its domain path identifies a finding while you triage it.
