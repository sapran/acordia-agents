...
Each specialist's `description` SHALL state its domain in one sentence, conveying the same routing signal as its CyberStrike counterpart's description, because `description` is the only routing signal a subagent has.
