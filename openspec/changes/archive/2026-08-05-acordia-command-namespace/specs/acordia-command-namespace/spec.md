...
The repository SHALL carry one slash-command wrapper per agent, at `commands/acordia/<stem>.md`, where `<stem>` is that agent's filename stem under `<pillar>/agents/`. The set SHALL be exactly 1:1 with the union of every pillar's agents, so a command name has a single source of truth and cannot drift from the agent it dispatches.
...
#### Scenario: One wrapper per agent, named for it

- **WHEN** `commands/acordia/` is compared with the union of `analysts/agents/*.md` and `operators/agents/*.md`
- **THEN** there is exactly one wrapper per agent and its filename stem equals the agent's filename stem
- **AND** no wrapper exists for a name that is not an agent
