...
- **Granting `webfetch`/`websearch` widens the opencode analyst posture.** It does not widen omp, where `BASE_TOOLS` already carries `web_search`. Read-only in this distribution has always meant "holds no file-editing tool", and fetching is collection, not modification. The alternative — narrowing `fusion-analyst`'s body claim about open sources — would have made the prompt describe less than the role model does.
