...
- [x] 1.1 Add `ownership` to the classification schema table in
...
- [x] 1.2 Rewrite the `## Guardrails` section of the same file as the four routing rules plus the
...
- [x] 1.3 Update triage step 5 to classify ownership before the rest of the schema and step 8 to state
...
- [x] 1.4 Add `doctrine_source: [Monte#operational-security, Heuer#information-quantity]` to the
  skill's `metadata.acordia` block; verify both keys resolve in `docs/roles/sources.md`
- [x] 1.5 Update the "record the pattern matched and the source location — never the raw value" line in
...
- [x] 2.1 Rewrite the closing cross-cutting line of `## Credential extraction` in
...
- [x] 2.2 Give `os-host-internals` the same closing discipline, which today defers to the triage skill
  without stating the rule at all; verify its cross-cutting line now states where extraction output
  goes
- [x] 2.3 Reconcile `web-api-authflow-analysis`'s "do not archive contents that are themselves
...
- [x] 3.1 Invert the credential carve-out in `briefing-reporting`'s "Cite identifiers in full" bullet
...
- [x] 3.2 In the same edit, add the verification carve-out to the rendered-product bullet: verify
...
- [x] 4.1 Update `exhaustive-data-processing`'s "No raw credential values in output" guardrail to the
  routing rule; verify it names working notes and the product as the destinations and keeps its
  passive-posture bullet
- [x] 4.2 Update `packet-traffic-analysis`'s "Never quote raw credential values — cite location only"
  to the routing rule; verify the line still requires a location citation
- [x] 4.3 Update `aleph-entity-graph`'s report bullet so a target-owned credential may be reported
  while bulk document text and excess personal identifiers stay excluded; verify the three clauses are
  still distinguishable
- [x] 4.4 Confirm `operational-memory` is unchanged and still forbids a secret unconditionally; verify
  by diffing the file against `origin/develop` and seeing no change

## 5. The five prompts and two wrappers

- [x] 5.1 Rewrite the `## Credential harvest` section in all five agent prompts to state
...
- [x] 5.2 Propagate the `cyber-analyst` body byte-identically into `commands/analyst.md` and
...
- [x] 6.1 Add the empty-literature gap to `docs/roles/sources.md` under *Gaps — searched and not
  found*, naming what was searched; verify it follows the two existing entries' shape
- [x] 6.2 Bump 6.7.0 → 6.8.0 in all three occurrences; verify with the version-lockstep check in
  `tools/check-acordia.sh`
- [x] 6.3 Run `tools/check-acordia.sh` in the worktree and the four by-hand invariant checks from
...
- [x] 6.4 Run `openspec validate --all --strict`; verify it passes
