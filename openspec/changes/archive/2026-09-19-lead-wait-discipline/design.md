...
**Forecast working budget: 109 + 323 = 432 characters.** *Measured on implementation: only 266 were freed, because each removed enumeration left a short routing pointer behind, giving a real budget of 375. The three clauses used 371; the final body is 10,491, with 9 characters of margin.* The forecast is kept here beside the measurement because the gap between them is the lesson: raw sentence length is not freed length when the sentence is replaced rather than deleted.
