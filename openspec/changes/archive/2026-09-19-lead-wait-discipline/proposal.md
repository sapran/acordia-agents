...
- Free the characters the three clauses need by relocating a **duplicated** enumeration. The orchestrator body was **10,391 characters against the 10,500 ceiling — 109 characters of headroom**, measured the way the gate measures it (`len(parts[2].strip())` after the frontmatter, not `wc -c` on the file). Three clauses do not fit in 109. The credential brief's required contents were enumerated twice in that body, so both moved to `credential-harvest-triage`, which owns the procedure. **Measured outcome: 266 characters freed rather than the 323 the raw sentence lengths suggested, because each removal left a short routing pointer behind; the clauses used 366 of the resulting 375; final body 10,491, with 9 characters of margin.** This is the remedy the ceiling requirement prescribes — reduce by moving technique detail to the skill that owns it — applied to prose that already existed rather than to prose this change introduces.
