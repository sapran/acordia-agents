...
**Reason**: The five operations prompts it governs are deleted with `acordia-operators/`, and no shipped agent makes target contact, so there is no authorized scope to gate and no `.acordia/ops/scope.md` to record it in.

**Migration**: None. The evidence-integrity half of the posture it shared with the analysts survives in `agent-roster`'s write-posture requirement; the authorization half has no shipped agent left to bind.

### Requirement: The journal contract is named once, not restated per prompt

**Reason**: The `operation-journal` skill that held the contract and the `.acordia/ops/` layout it described are deleted with the operations skill library, along with every prompt that named them.

**Migration**: None. An analyst product goes to `.acordia/reports/` by convention, per `agent-roster`'s report-sink requirement; no shipped agent records operation state.

### Requirement: Every operations prompt names the remote-execution posture

**Reason**: `bolts` and the five prompts required to name it are deleted with the pillar; nothing in the distribution originates traffic, so there is no posture deciding where it comes from.

**Migration**: None. The distinction it protected — a cross-cutting posture is not a specialist depth — survives in `agent-roster`'s skill-line requirement, which fixes what each heading claims.

## RENAMED Requirements

- FROM: `### Requirement: Nine agents, two pillars, one authored file each`
- TO: `### Requirement: Five agents, one pillar, one authored file each`
