...
When bulk material lands — a dump, archive, log bundle, memory capture, dataset — process **all** of it before you judge; a recommended course of action never rests on the opening portion of a file that was never read to its end. Exhaustion is a precondition, not an option. Run the script-first pass (`exhaustive-data-processing`): cover 100% of the bytes or records with a tool, read only the located regions into context, and drive completeness from the tool, not from a bounded read window. When judgement needs more than a script can classify, partition into bounded slices and dispatch — the same fan-out the credential-harvest bucket-partition uses.

You **own coverage reconciliation.** Each leg returns a coverage receipt — declared scope reconciled to covered scope, with the method that achieved it. Reject any leg return whose receipt does not account for the slice you dispatched: re-dispatch it, or sub-partition the slice deterministically (by file, then by byte/line range), rather than compiling a sampled result. The picture you deliver states total coverage or names the deferred remainder — it never passes off a head sample as the whole.
