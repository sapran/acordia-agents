...
| omp | `tools` allowlist omitting `edit` and `task`; `write` is omitted for `target-network-analyst` and `defender-detection-analyst`, and named outright for `operational-analyst` and `fusion-analyst`, which must produce reports | **partial** — `edit` and `task` really are removed; `write` survives where it is omitted, and is granted deliberately for the two reporting analysts |
...
- **omp cannot deny `write`.** For the two analysts whose allowlist omits it — `target-network-analyst` and `defender-detection-analyst` — omp exposes it anyway: `read` and `write` are omp's `XDEV_TRANSPORT_TOOLS`, the channel every `xd://` device is driven through, so they are present whenever the `tools.xdev` setting is on — which is the default. Verified empirically against omp 17.1.8: a leg agent asked to create a scratch file with `write` succeeded. Disable `tools.xdev` in omp's settings and the allowlist bites for those two. It does **not** bite for `operational-analyst` and `fusion-analyst`: their allowlist names `write` by design, because their prompts require them to produce reports, so for them the setting changes nothing and the capability is intended rather than leaked. The generated frontmatter records which case each agent is in, under `metadata.generated.write_access`, rather than implying a guarantee it cannot keep.
